...
class EthicsModel:
    ...
    def __init__(self, model, load_path, load_dir=None, oracle=False):
        ...
        for name, parma in self.cclm.named_parameters():
            if not name.startswith("classifier"):
                parma.requires_grad = False
        self.cclm_optimizer = AdamW(self.cclm.parameters(), lr=1e-5, eps=1e-8)
    ...
    def train_cclm(self, data_loader):
        self.cclm.train()
        ...
        for step, batch in enumerate(data_loader):
            ...
            self.cclm_optimizer.zero_grad()

            # Forward pass
            output = self.cclm(b_input_ids, attention_mask=b_input_mask)[0].squeeze()

            loss = criterion(output, b_moral_values.float())

            # Backward pass
            loss.backward()

            # Update weights
            self.cclm_optimizer.step()

    def get_prob(self, input_ids, input_mask):
        self.cclm.eval()
        with torch.no_grad():
            output = self.cclm(input_ids, attention_mask=input_mask)[0]
        if self.morality_mode == 'cm':
            prob = torch.sigmoid(output).squeeze(dim=1)
        elif self.morality_mode == 'util':
            prob = output.squeeze(dim=1)
        return prob
