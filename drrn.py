...
def build_state_with_threshold(lm, obs, infos, envs, prev_obs, prev_acts, state_length_threshold):
    """
    Build the state with threshold, if the state is too long, simplify it!
    """
    assert (prev_obs is not None) and (prev_acts is not None), '[XYQ] [20220530] The prev_obs / prev_acts should not be none!'
    states = []
    for prev_ob, ob, info, act, env in zip(prev_obs, obs, infos, prev_acts, envs):
        sent = "[CLS] %s [SEP] %s [SEP] %s [SEP]" % (prev_ob, act, ob + info['inv'] + info['look'])
        encoded_state = lm.sent2ids(sent)
        s1,s2,s3,s4 = len(encoded_state), None, None, None
        # Simplify 1
        if len(encoded_state) > state_length_threshold:
            sent = "[CLS] %s [SEP] %s [SEP] %s [SEP]" % (prev_ob, act, ob + info['inv'])
            encoded_state = lm.sent2ids(sent)
            s2 = len(encoded_state)
        # Simplify 2
        if len(encoded_state) > state_length_threshold:
            sent = "[CLS] %s [SEP] %s [SEP]" % (act, ob + info['inv'])
            encoded_state = lm.sent2ids(sent)
            s3 = len(encoded_state)
        
        # Simplify 3
        if len(encoded_state) > 2 * state_length_threshold:
            sent = "[CLS] %s [SEP] %s [SEP]" % (act, ob + info['inv'])
            encoded_state = lm.sent2ids(sent, maxlen= 2 * state_length_threshold)
            s4 = len(encoded_state)
        
        states.append(State(lm.sent2ids(ob), lm.act2ids(info['look']), lm.act2ids(info['inv']), encoded_state, sent, hash(str(env.env.get_state()))))
    return states
